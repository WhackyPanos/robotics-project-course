...
double effort_array[] = {
    0, 0, 0,
    0, 0, 0}; // effort array (just needed to populate message component)
...
// 	const std_msgs__msg__Int64MultiArray * one_servo = (const
// std_msgs__msg__Int64MultiArray *)msgin;

// 	// TODO: Add condition that servo pos is positive (i.e. dont do anything
// if it negative) 	if (one_servo->data.data[0] == 1) {
// 		servo1.move_time(one_servo->data.data[1],
// one_servo->data.data[2]);
// 	}
// 	else if (one_servo->data.data[0] == 2) {
// 		servo2.move_time(one_servo->data.data[1],
// one_servo->data.data[2]);
// 	}
// 	else if (one_servo->data.data[0] == 3) {
// 		servo3.move_time(one_servo->data.data[1],
// one_servo->data.data[2]);
// 	}
// 	else if (one_servo->data.data[0] == 4) {
// 		servo4.move_time(one_servo->data.data[1],
// one_servo->data.data[2]);
// 	}
// 	else if (one_servo->data.data[0] == 5) {
// 		servo5.move_time(one_servo->data.data[1],
// one_servo->data.data[2]);
// 	}
// 	else if(one_servo->data.data[0] == 6) {
// 		servo6.move_time(one_servo->data.data[1],
// one_servo->data.data[2]);
// 	}
// 	else {
// 		digitalWrite(LED_BUILTIN, LOW);
...
void subscription_callback_multi_servo(const void *msgin) {
  ...
  const std_msgs__msg__Int16MultiArray *multi_servo =
...
bool create_entities() {
  ...
  // micro-ROS allocator RCCHECK(rclc_support_init(&support, 0, NULL,
  // &allocator));					// Initialize support
  // options
  // 	RCCHECK(rclc_node_init_default(&node_hiwonder, "Hiwonder_xArm_node", "",
  // &support)); // create node
  ...
  RCCHECK(rclc_subscription_init_default(
      &subscriber_multi_servo_cmd, &node_hiwonder,
      ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int16MultiArray),
      "multi_servo_cmd_sub"));
  ...
  // init executors
  RCCHECK(rclc_executor_init(&executor_servo_pos_publish, &support.context, 1,
                             &allocator)); // executor for publishing servo pos.
  RCCHECK(rclc_executor_init(&executor_servo_vin_publish, &support.context, 1,
                             &allocator)); // executor for publishing servo pos.
  RCCHECK(rclc_executor_init(&executor_servo_temp_publish, &support.context, 1,
                             &allocator)); // executor for publishing servo pos.
  // RCCHECK(rclc_executor_init(&executor_single_servo_cmd_sub,
  // &support.context, 1, &allocator));		// executor for one servo cmd
  // subscriber
  RCCHECK(rclc_executor_init(
      &executor_multi_servo_move_cmd_sub, &support.context, 1,
      &allocator)); // executor for multi servo cmd subscriber

  // executor additions
  // RCCHECK(rclc_executor_add_subscription(&executor_single_servo_cmd_sub,
  // &subscriber_one_servo_cmd, &single_servo_cmd_msg_in,
  // &subscription_callback_single_servo, ON_NEW_DATA));
  RCCHECK(rclc_executor_add_subscription(
      &executor_multi_servo_move_cmd_sub, &subscriber_multi_servo_cmd,
      &multi_servo_cmd_msg_in, &subscription_callback_multi_servo,
      ON_NEW_DATA));
...
void setup() {
  ...
  bool success =
      rosidl_runtime_c__String__Sequence__init(&name__string_sequence, 6);
  servo_position_array_msg.name = name__string_sequence;
  success = rosidl_runtime_c__String__assignn(
      &servo_position_array_msg.name.data[0], "Servo1", 6);
  success = rosidl_runtime_c__String__assignn(
      &servo_position_array_msg.name.data[1], "Servo2", 6);
  success = rosidl_runtime_c__String__assignn(
      &servo_position_array_msg.name.data[2], "Servo3", 6);
  success = rosidl_runtime_c__String__assignn(
      &servo_position_array_msg.name.data[3], "Servo4", 6);
  success = rosidl_runtime_c__String__assignn(
      &servo_position_array_msg.name.data[4], "Servo5", 6);
  success = rosidl_runtime_c__String__assignn(
      &servo_position_array_msg.name.data[5], "Servo6", 6);
  ...
  servo_position_array_msg.position.data = (double *)malloc(
      servo_position_array_msg.position.capacity * sizeof(double));
  ...
  servo_position_array_msg.effort.data = (double *)malloc(
      servo_position_array_msg.position.capacity * sizeof(double));
  ...
  servo_position_array_msg.velocity.data = (double *)malloc(
      servo_position_array_msg.position.capacity * sizeof(double));
  ...
  servo_position_array_msg.header.frame_id.data = (char *)malloc(
      servo_position_array_msg.header.frame_id.capacity * sizeof(char));
  ...
  servo_position_array_msg.header.frame_id.size =
      strlen(servo_position_array_msg.header.frame_id.data);

  // Assigning time stamp
  servo_position_array_msg.header.stamp.sec =
      (uint16_t)(rmw_uros_epoch_millis() / 1000);
  servo_position_array_msg.header.stamp.nanosec =
      (uint32_t)rmw_uros_epoch_nanos();
  ...
  // single_servo_cmd_msg_in.layout.dim.data =
  // (std_msgs__msg__MultiArrayDimension*) malloc(3 *
  ...
  // 	single_servo_cmd_msg_in.layout.dim.data[i].label.data = (char*)
  // malloc(single_servo_cmd_msg_in.layout.dim.data[i].label.capacity *
  // sizeof(char));
  // }
  ...
  multi_servo_cmd_msg_in.data.data = (int16_t *)malloc(
      single_servo_cmd_msg_in.data.capacity * sizeof(int16_t));

  multi_servo_cmd_msg_in.layout.dim.capacity = 3;
  multi_servo_cmd_msg_in.layout.dim.size = 0;
  multi_servo_cmd_msg_in.layout.dim.data =
      (std_msgs__msg__MultiArrayDimension *)malloc(
          multi_servo_cmd_msg_in.layout.dim.capacity *
          sizeof(std_msgs__msg__MultiArrayDimension));
  ...
  for (size_t i = 0; i < multi_servo_cmd_msg_in.layout.dim.capacity; i++) {
    ...
    multi_servo_cmd_msg_in.layout.dim.data[i].label.data = (char *)malloc(
        multi_servo_cmd_msg_in.layout.dim.data[i].label.capacity *
        sizeof(char));
  }
  ...
  servo_voltages_msg.data.data =
      (int16_t *)malloc(servo_voltages_msg.data.capacity * sizeof(int16_t));

  servo_voltages_msg.layout.dim.capacity = 3;
  servo_voltages_msg.layout.dim.size = 0;
  servo_voltages_msg.layout.dim.data =
      (std_msgs__msg__MultiArrayDimension *)malloc(
          servo_voltages_msg.layout.dim.capacity *
          sizeof(std_msgs__msg__MultiArrayDimension));
  ...
  for (size_t i = 0; i < servo_voltages_msg.layout.dim.capacity; i++) {
    ...
    servo_voltages_msg.layout.dim.data[i].label.data = (char *)malloc(
        servo_voltages_msg.layout.dim.data[i].label.capacity * sizeof(char));
  }

  // (5) servo_temps_msg
  servo_temps_msg.data.capacity = 6;
  servo_temps_msg.data.size = 6;
  servo_temps_msg.data.data =
      (int16_t *)malloc(servo_temps_msg.data.capacity * sizeof(int16_t));

  servo_temps_msg.layout.dim.capacity = 3;
  servo_temps_msg.layout.dim.size = 0;
  servo_temps_msg.layout.dim.data =
      (std_msgs__msg__MultiArrayDimension *)malloc(
          servo_temps_msg.layout.dim.capacity *
          sizeof(std_msgs__msg__MultiArrayDimension));
  ...
  for (size_t i = 0; i < servo_temps_msg.layout.dim.capacity; i++) {
    ...
    servo_temps_msg.layout.dim.data[i].label.data = (char *)malloc(
        servo_temps_msg.layout.dim.data[i].label.capacity * sizeof(char));
  }
}
...
void loop() {
  ...
  RCSOFTCHECK(rclc_executor_spin_some(&executor_multi_servo_move_cmd_sub,
                                      RCL_MS_TO_NS(100)));
  ...
  case AGENT_CONNECTED:
    ...
    if (state == AGENT_CONNECTED) {
      if (publish_servo_pos_flag == true) {
        ...
        servo_position_array_msg.velocity.data = vel;
        servo_position_array_msg.header.stamp.sec =
            (uint16_t)(rmw_uros_epoch_millis() / 1000); // timestamp
        servo_position_array_msg.header.stamp.nanosec =
            (uint32_t)rmw_uros_epoch_nanos(); // timestamp

        // Publishes
        RCSOFTCHECK(
            rcl_publish(&publisher_servo_pos, &servo_position_array_msg, NULL));
      ...
      if (publish_temp_and_volt_flag == true) {
        ...
        // Publish messages
        RCSOFTCHECK(
            rcl_publish(&publisher_servo_vin, &servo_voltages_msg, NULL));
        RCSOFTCHECK(rcl_publish(&publisher_servo_temp, &servo_temps_msg, NULL));

        publish_temp_and_volt_flag = false;
      }
      rclc_executor_spin_some(&executor_multi_servo_move_cmd_sub,
                              RCL_MS_TO_NS(100));
    }
    break;
